...
const getEnv = module.exports._getEnv = function (args) {
    ...
    for (let i = MIN_ARG; i < nbArgs; ++i) {
        if (typeof args[i] === 'object' && !Array.isArray(args[i]) && args[i].hasOwnProperty('env')) {
    ...
    executePre(args) {
        ...
        for (let i = 0; i < this.matcherList.length; ++i) {
            ...
            for (let j = 0; j < envList.length; ++j) {
                ...
                if (this.matcherList[i].match(envList[j])) {

                    const result = { record: { found: this.matcherList[i].pattern } };
                    if (this.rule.block) {
